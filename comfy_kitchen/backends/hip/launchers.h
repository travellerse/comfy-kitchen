...
void launch_int8_gemm_dp4a_kernel(const void* a, const void* b, void* c, const void* scale_a,
                                  ...
                                  hipStream_t stream);
// RDNA2 W4A4 GEMM (ops/gemm_w4a4_dp4a.hip): packed int4 operands, K/2 bytes per
// row, sdot8 inner loop. Weight scales are per-output-channel only (the W4A4
// quantizer emits one scale per row), so the epilogue stride is fixed at 1.
void launch_w4a4_gemm_dp4a_kernel(const void* a, const void* b, void* c, const void* scale_a,
                                  const void* scale_b, const void* bias, int bias_code,
